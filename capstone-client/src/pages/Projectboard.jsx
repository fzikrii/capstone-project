...
// --- Mock Data ---
// This is placeholder data. Eventually, it will come from your database (MongoDB).
const userData = {
  name: 'Jonathan Ezra Navarro',
  title: 'Project Manager',
  avatarUrl: 'https://placehold.co/100x100/6366F1/FFFFFF?text=JN',
  stats: {
    tasksCompleted: 78,
...
const projectData = {
  'todo': {
    ...
    color: 'bg-slate-500',
    tasks: [
      { id: 'task-1', title: 'Redesign user login flow', tags: ['UI/UX', 'Important'] },
      { id: 'task-2', title: 'Prepare API endpoint for profile', tags: ['Backend'] },
    ],
  },
  'ongoing': {
    name: 'Ongoing',
    color: 'bg-blue-500',
    tasks: [
      { id: 'task-3', title: 'Implement React Sidebar component', tags: ['Frontend', 'High Priority'] },
      { id: 'task-4', title: 'Design database schema for the project', tags: ['Database'] },
      { id: 'task-5', title: 'Create Task Card component', tags: ['Frontend'] },
    ],
  },
  'done': {
    name: 'Done',
    color: 'bg-green-500',
    tasks: [
      { id: 'task-6', title: 'Setup Next.js project with Tailwind', tags: ['Setup'] },
      { id: 'task-7', title: 'Create initial design in Figma', tags: ['Design'] },
    ],
  },
  'stuck': {
    name: 'Stuck',
    color: 'bg-red-500',
    tasks: [
      { id: 'task-8', title: 'Integration with third-party payment API', tags: ['API', 'Help'] },
    ],
  },
};

const faqData = [
  { q: 'How do I create a new project?', a: 'Click the "My Project" button on the sidebar, then find the "+ New Project" button on the project page. You will be asked to fill in the project name and description.' },
  { q: 'Can I invite team members to my project?', a: 'Absolutely. On every project page, there is a "Members" tab. From there, you can invite other users via their email address.' },
  { q: 'How do profile banner tags work?', a: 'Banner tags are awarded automatically based on your contributions. For example, completing a number of tasks will give you the "Fast Finisher" banner, and being a top contributor will earn you the "Top Contributor" banner.' },
  { q: 'Is my data safe?', a: 'We use industry-standard encryption and best security practices to ensure your data is always safe and protected.' },
];

// --- Application Components ---

// Task Card Component for the Kanban Board
const TaskCard = ({ task }) => (
...
const ProfileModal = ({ user, onClose }) => {
  const getContributionLevel = (tasks) => {
    if (tasks > 75) return { name: "Master Contributor", color: "bg-purple-500", icon: <Award size={16} /> };
    if (tasks > 50) return { name: "Fast Finisher", color: "bg-green-500", icon: <Zap size={16} /> };
    if (tasks > 10) return { name: "Active Contributor", color: "bg-blue-500", icon: <BarChart2 size={16} /> };
    return { name: "Beginner", color: "bg-slate-500", icon: <User size={16} /> };
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50" onClick={onClose}>
      <div
        className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl p-6 w-full max-w-sm transform transition-all"
      ...
      >
        <div className="flex flex-col items-center text-center">
          <img
            src={user.avatarUrl}
            alt={user.name}
            className="w-24 h-24 rounded-full ring-4 ring-indigo-300 dark:ring-indigo-500 mb-4"
          />
        ...
        <div className="mt-6 space-y-3">
          <h3 className="text-sm font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider">Achievement Banner</h3>
          ...
          {user.stats.topContributorIn > 0 && (
            <div className="flex items-center gap-3 bg-amber-400 text-slate-800 p-3 rounded-lg">
              <Award size={16} />
              <span className="font-semibold">Top Contributor in {user.stats.topContributorIn} Projects</span>
            </div>
          )}
        </div>
        <div className="mt-6">
          <h3 className="text-sm font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-2">Statistics</h3>
        ...
        <button
          onClick={onClose}
          className="mt-6 w-full bg-indigo-500 hover:bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg transition-colors"
        >
          Close
        </button>
      </div>
...
// Page Component: Project (Kanban)
const ProjectView = () => (
  <div className="p-4 sm:p-6 lg:p-8">
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <h1 className="text-3xl font-bold text-slate-800 dark:text-white">Project's Board</h1>
...
// Page Component: Board (Bounty Board)
const BoardView = () => (
    <div className="p-4 sm:p-6 lg:p-8">
        <h1 className="text-3xl font-bold text-slate-800 dark:text-white mb-6">Bounty Board</h1>
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-6">
            <p className="text-slate-600 dark:text-slate-300">
                This page displays all the tasks ("bounties") from various projects assigned to you. It helps you focus on what needs to be done next without switching between projects.
            </p>
            <div className="mt-4 border-t border-slate-200 dark:border-slate-700 pt-4">
                <h3 className="font-semibold text-lg text-slate-700 dark:text-slate-200">Sample Tasks Assigned to You:</h3>
                <ul className="list-disc list-inside mt-2 space-y-1 text-slate-500 dark:text-slate-400">
                    <li>[QuantumLeap Project] Implement Sidebar React component</li>
                    <li>[Phoenix Project] Design database schema for the project</li>
                    <li>[Titan Project] Integrate with payment API</li>
                </ul>
            </div>
        </div>
    </div>
...
const ScheduleView = () => (
    <div className="p-4 sm:p-6 lg:p-8">
        <h1 className="text-3xl font-bold text-slate-800 dark:text-white mb-6">Schedule</h1>
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-6">
            <p className="text-slate-600 dark:text-slate-300 mb-4">
                This section displays your calendar integrated with Google Calendar. All task deadlines and project events will appear automatically.
            </p>
            <div className="aspect-w-16 aspect-h-9 bg-slate-100 dark:bg-slate-700 rounded-lg flex items-center justify-center">
                <img src="https://placehold.co/800x450/E2E8F0/475569?text=Google+Calendar+Integration" alt="Calendar Placeholder" className="rounded-lg" />
            </div>
        </div>
    </div>
);
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="hidden sm:block">
                        <input
                            ...
                            className="w-48 bg-slate-100 dark:bg-slate-800 rounded-md px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                        />
                    </div>
                </div>
            </div>
